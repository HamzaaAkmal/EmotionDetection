...
class FingerprintManager:
    ...
    def generate_user_agents_gemini(self, browser_id, update_signal) -> bool:
        ...
        try:
            update_signal.emit(f"Bot {browser_id + 1}: Contacting Gemini API (model: gemini-2.0-flash)...", browser_id)
            genai.configure(api_key=self.gemini_api_key)
            model = genai.GenerativeModel('gemini-2.0-flash') # Use the specified model
            ...
            if "api key not valid" in err_str:
                 ...
                 update_signal.emit(f"Bot {browser_id + 1}: Gemini Error: Model 'gemini-2.0-flash' not found.", browser_id)
            else:
